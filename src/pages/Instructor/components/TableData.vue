...
const hasNext = computed(() => dt_resp.value.totalPages > currentPage.value + 1);

const pageNumbers = computed(() => {
    const totalPages = dt_resp.value.totalPages || 0;
    const startPage = Math.floor(currentPage.value / 5) * 5;
    return Array.from({ length: Math.min(5, totalPages - startPage) }, (_, i) => startPage + i);
});

const fetchDt = async () => {
    await instructorStore.fetchDt(currentPage.value, pageSize);
    if (instructorStore.dt_res.isSuccess) {
...
<template>
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg w-full">
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3 text-center" style="width: 20%;">자료명</th>
            ...
            <tbody>
                <tr v-for="dt in dt_resp.boardList" :key="dt.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    ...
                    <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white text-center" @click="openPopup(dt)" style="width: 20%;">{{ dt.title }}</th>
                    <td class="px-6 py-4 text-center" @click="openPopup(dt)" style="width: 30%;">{{ dt.fileName }}</td>
                    <td class="px-6 py-4 text-center" @click="openPopup(dt)" style="width: 15%;">{{ new Date(dt.createdDate).toLocaleDateString() }}</td>
                    <td class="px-6 py-4 text-center" @click="openPopup(dt)" style="width: 15%;">{{ dt.writer }}</td>
                    <td class="px-6 py-4 flex space-x-2 text-center" style="width: 15%;">
                        ...
                        <button @click="openPopup(dt.fileName)" class="inline-flex items-center p-2 text-sm font-medium text-center text-gray-900 bg-white rounded-lg hover:bg-gray-100 focus:ring-4 focus:outline-none dark:text-white focus:ring-gray-50 dark:bg-gray-800 dark:hover:bg-gray-700 dark:focus:ring-gray-600" type="button">
                            <svg class="w-4 h-5 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="none" viewBox="0 0 24 24">
        ...
        <nav class="flex items-center flex-column flex-wrap md:flex-row justify-between pt-4 px-4" aria-label="Table navigation">
            <span class="text-sm font-normal text-gray-500 dark:text-gray-400 mb-4 md:mb-0 block w-full md:inline md:w-auto">Showing <span class="font-semibold text-gray-900 dark:text-white">{{ currentPage * pageSize + 1 }}-{{ Math.min((currentPage + 1) * pageSize, dt_resp.totalElements) }}</span> of <span class="font-semibold text-gray-900 dark:text-white">{{ dt_resp.totalElements }}</span></span>
    ...
    <div v-if="showPopup" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="relative p-4 w-full max-w-md max-height-full">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                <button type="button" @click="closePopup" class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white">
                ...
                <div class="p-4 md:p-5 text-center">
                    ...
                    <button type="button" @click="closePopup" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center">
                        Yes, I'm sure
                    </button>
                    <button type="button" @click="closePopup" class="py-2.5 px-5 ms-3 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">No, cancel</button>
                </div>
            </div>
        </div>
    </div>
